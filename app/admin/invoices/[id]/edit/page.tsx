...
// Temporaire : à remplacer par API
const clients = [
    { id: '1', name: 'Client A' },
    { id: '2', name: 'Client B' },
];
...
export default function EditInvoiceForm() {
    ...
    const [invoice, setInvoice] = useState<InvoiceData | null>(null); // ✅ Typé
    ...
    const handleProductChange = (index: number, key: string, value: string | number) => {
        const updated = [...products];
        updated[index][key] = key === 'quantity' || key === 'unitPrice' ? Number(value) : value;
            ...
            <div className="space-y-4 p-6 rounded shadow border text-black bg-white">
                ...
                <div className='grid grid-cols-1 sm:grid-cols-2 gap-4'>
                    <div>
                        <div className='flex items-center gap-2'>
                            <Label>TVA</Label><Switch checked={tvaEnabled} onCheckedChange={setTvaEnabled} />
                        </div>
                ...
                {products.map((p, i) => (
                    <div key={i} className="flex gap-2">
                        <Input value={p.description} onChange={(e) => handleProductChange(i, 'description', e.target.value)} />
                        <Input type="number" value={p.quantity} onChange={(e) => handleProductChange(i, 'quantity', e.target.value)} />
                        <Input type="number" value={p.unitPrice} onChange={(e) => handleProductChange(i, 'unitPrice', e.target.value)} />
                        <div>{p.total.toFixed(2)} €</div>
                        <button className='text-red-600' onClick={() => setProducts(products.filter((_, idx) => idx !== i))}><Trash2 size={20} /></button>
                    </div>
                ))}
